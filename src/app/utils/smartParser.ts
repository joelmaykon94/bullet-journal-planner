...
export interface ParsedSmartTask {
  ...
  endDate?: string; // YYYY-MM-DD (optional end date for recurrence range)
  time?: string; // HH:MM (start)
  endTime?: string; // HH:MM (end)
  priority?: boolean;
  energy?: number;
  complexity?: number;
  recurrence?: 'daily' | 'weekdays' | 'weekly' | 'monthly' | 'custom-weekly';
  recurrenceDays?: number[]; // 0=domingo, 1=segunda, ... 6=sábado
  reminderType?: 'notification' | 'email' | 'both';
  constantReminder?: boolean;
}

export const parseSmartTask = (
  input: string,
  ...
  let recurrence: ParsedSmartTask['recurrence'] = undefined;
  let recurrenceDays: number[] | undefined = undefined;
  let reminderType: ParsedSmartTask['reminderType'] = undefined;
  let constantReminder: boolean | undefined = undefined;
  ...
  const replacePreservingEdges = (re: RegExp) => {
  ...
  const parseNamedDate = (dayStr: string, monthStr: string, yearStr?: string): string | undefined => {
  ...
  const parseSlashDate = (dd: string, mm: string, yyyy?: string): string | undefined => {
    const day = Number(dd);
  ...
  if (/\balarme\b/i.test(cleanContent)) {
    ...
    if (!reminderType) reminderType = 'notification';
  }
  ...
  const timeRangeRegex =
    /(?:às|as|ás)?\s*([0-2]?\d)(?:(?::([0-5]\d))|(?:h([0-5]\d)?))?\s*(?:at[eé]|a|-|–)\s*([0-2]?\d)(?:(?::([0-5]\d))|(?:h([0-5]\d)?))?/i;
  ...
  const namedRangeRegex =
    /\b(?:do\s+dia\s+)?(\d{1,2})\s+de\s+(janeiro|fevereiro|março|marco|abril|maio|junho|julho|agosto|setembro|outubro|novembro|dezembro)(?:\s+de\s+(\d{4}))?\s+at[eé]\s+(\d{1,2})\s+de\s+(janeiro|fevereiro|março|marco|abril|maio|junho|julho|agosto|setembro|outubro|novembro|dezembro)(?:\s+de\s+(\d{4}))?\b/i;
  ...
  if (!date) {
    const nextWeekdayRe = new RegExp(
      `${WORD_EDGE_PRE}(pr[oó]xim[oa])\\s+(?:no\\s+|na\\s+)?(segunda|terça|terca|quarta|quinta|sexta|sábado|sabado|domingo)(?:-feira)?${WORD_EDGE_POST}`,
      'iu'
    );
  ...
  if (/\b(todos os dias|diariamente)\b/i.test(lower)) {
    ...
    for (const [name, idx] of Object.entries(dayMap)) {
      if (lower.includes(name)) mentioned.push(idx);
    }
    if (mentioned.length > 0) {
      recurrence = 'custom-weekly';
      recurrenceDays = [...new Set(mentioned)].sort();
      replacePreservingEdges(
        new RegExp(
          `${WORD_EDGE_PRE}(segunda|terça|terca|quarta|quinta|sexta|sábado|sabado|domingo)(?:-feira)?${WORD_EDGE_POST}`,
          'giu'
        )
      );
    } else {
    ...
    if (!forceSingleWeekday) {
      const weekdayMentionRe = new RegExp(
        `${WORD_EDGE_PRE}(segunda|terça|terca|quarta|quinta|sexta|sábado|sabado|domingo)(?:-feira)?(s)?${WORD_EDGE_POST}`,
        'giu'
      );
      const matches = Array.from(cleanContent.matchAll(weekdayMentionRe));
      const mentionedIdx: number[] = [];
      const mentionedPlural: boolean[] = [];
      matches.forEach((m) => {
      ...
      if (uniqueDays.length >= 2) {
        ...
        if (isPlural || hasAsCue || hasTodaCue) {
          ...
          if (!date) date = getNextDayOfWeek(dayIdx);
          cleanContent = cleanContent.replace(weekdayMentionRe, '$1$4').trim();
        }
      }
    }
  ...
  if (!date) {
    // Single slash date: "15/08" or "15/08/2026" or "dia 15/08"
    const singleSlashRegex = /\b(?:dia\s+)?(\d{1,2})\/(\d{1,2})(?:\/(\d{4}))?\b/i;
    const sr = cleanContent.match(singleSlashRegex);
    if (sr) {
      const dt = parseSlashDate(sr[1], sr[2], sr[3]);
      if (dt) {
        date = dt;
        cleanContent = cleanContent.replace(singleSlashRegex, '').trim();
      }
    }
  }

  if (!date) {
    // Single named date: "15 de Agosto" or "15 de agosto de 2026"
    const singleNamedRegex = /\b(?:dia\s+)?(\d{1,2})\s+de\s+(janeiro|fevereiro|março|marco|abril|maio|junho|julho|agosto|setembro|outubro|novembro|dezembro)(?:\s+de\s+(\d{4}))?\b/i;
    const nr = cleanContent.match(singleNamedRegex);
    if (nr) {
      const dt = parseNamedDate(nr[1], nr[2], nr[3]);
      if (dt) {
        date = dt;
        cleanContent = cleanContent.replace(singleNamedRegex, '').trim();
      }
    }
  }
  ...
  if (!date) {
    const relativeRe = new RegExp(
      `${WORD_EDGE_PRE}(hoje|amanhã|amanha|depois de amanhã|depois de amanha)${WORD_EDGE_POST}`,
      'iu'
    );
    const relativeMatch = cleanContent.match(relativeRe);
    if (relativeMatch) {
      const word = relativeMatch[2].normalize('NFC').toLowerCase();
      if (word === 'hoje') date = referenceDate;
      else if (word === 'amanhã' || word === 'amanha') date = addDays(1);
      else date = addDays(2);
  ...
  if (!date) {
    const weekdaysRegex = new RegExp(
      `${WORD_EDGE_PRE}(segunda|terça|terca|quarta|quinta|sexta|sábado|sabado|domingo)(?:-feira)?${WORD_EDGE_POST}`,
      'iu'
    );
  ...
  cleanContent = cleanContent
    .replace(/\s{2,}/g, ' ')
    .replace(/\b(para|para o dia|no dia|de|e)\b\s*$/i, '')
    .replace(/,\s*$/, '')
    .replace(/(?:^|\s)(às|as|ás)\b\s*$/i, '')
    .replace(/^(?:\s*(?:\bde\b|\be\b|,|;|:)\s*)+/i, '')
    .replace(/^(?:\bàs\b|\bas\b|\bás\b)\s*/i, '')
    .replace(/^(?:\s*(?:\bàs\b|\bas\b|\bás\b)\s*)+/i, '')
    .replace(/(?:\s*(?:\bde\b|\be\b|,|;|:)\s*)+$/i, '')
    .replace(/\s{2,}/g, ' ')
    .trim();
