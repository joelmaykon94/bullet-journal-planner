...
export type SoundType = 'chuva_lareira' | 'lofi_jazz' | 'foco_marrom' | 'vento_floresta' | 'gurenge_theme' | 'tanjiro_theme' | 'homura_theme';

export function useAmbientAudio(showToast: (msg: string) => void) {
  const [soundType, setSoundType] = useState<SoundType>('chuva_lareira');
