      ...
      <div className="rounded-3xl bg-zinc-200/20 dark:bg-zinc-900/30 border border-zinc-200/30 dark:border-white/5 p-4 flex flex-col gap-3 w-full">
        <div className="flex items-center justify-between border-b border-zinc-200/20 dark:border-white/5 pb-2">
          <h4 className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest flex items-center gap-1.5">
            ...
            Hábitos de Hoje pelos Sonhos
          </h4>
          <button
            onClick={() => setActiveTab('dream_board')}
            className="text-[9px] font-bold text-bujo-highlight hover:underline cursor-pointer"
          >
            Gerenciar nos Sonhos
          </button>
        </div>

        {/* Small general stats dashboard */}
        {habits.length > 0 && (
          <div className="grid grid-cols-2 gap-2 bg-zinc-200/5 dark:bg-white/[0.01] p-2 rounded-2xl border border-zinc-200/10 dark:border-white/5 text-center">
            <div className="flex flex-col items-center">
              <span className="text-[8px] font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Adesão Geral</span>
              <span className="text-xs font-black text-bujo-highlight font-mono mt-0.5">
                {(() => {
                  const get8Days = () => {
                  ...
                  const last8Days = get8Days();
                  let totalCompletions = 0;
                  habits.forEach(h => {
                    last8Days.forEach(d => {
                      if (habitLogs[h]?.[d]) totalCompletions++;
                    });
                  });
                  const maxPossible = habits.length * 8;
                  return maxPossible > 0 ? Math.round((totalCompletions / maxPossible) * 100) : 0;
                })()}%
              </span>
            </div>
            <div className="flex flex-col items-center">
              <span className="text-[8px] font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Consistência Diária</span>
              <span className="text-xs font-black text-emerald-500 font-mono mt-0.5">
                {habits.filter(h => habitLogs[h]?.[today]).length} de {habits.length}
              </span>
            </div>
          </div>
        )}

        {/* Flat Habits list with dream legend and day count */}
        <div className="flex flex-col gap-2 max-h-[220px] overflow-y-auto pr-1">
          {habits.length === 0 ? (
            <div className="text-center py-6 text-zinc-500 italic text-[10px]">
              Nenhum hábito cadastrado. Crie um hábito relacionado a um sonho no Quadro de Sonhos!
            </div>
          ) : (
            habits.map(habit => {
              const isChecked = habitLogs[habit]?.[today] || false;
              const associatedDreamId = habitDreamMap[habit];
              const dream = dreams.find(d => d.id === associatedDreamId);
              
              // Count completed days in habit history (overall total checked days)
              const completedDaysCount = habitLogs[habit]
                ? Object.values(habitLogs[habit]).filter(Boolean).length
                : 0;
                ...
                >
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    ...
                    <div className="min-w-0 flex flex-col gap-0.5">
                      ...
                      {dream ? (
                        <span className="text-[7.5px] text-zinc-500 font-bold uppercase tracking-wider flex items-center gap-1">
                          <span>🎯</span>
                          <span className="truncate">Sonho: {dream.icon} {dream.title}</span>
                        </span>
                      ) : (
