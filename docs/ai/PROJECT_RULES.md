...
## Tester Discipline & Anti-Regression (TDD Cycles)

To ensure generated code never breaks existing system behavior or introduces hidden property/type errors:

1. **Act as a Tester**: Never assume code changes "just work". Actively seek out property mismatches, compile breaks, or invalid data types in sibling files.
2. **Double-Ended Validation**: Always verify endpoints by comparing structural data properties between backend controller DTOs/Entities and frontend model/service/component expectations.
3. **Verify the Whole System**:
   - Run complete backend check: `pnpm --filter backend build && pnpm --filter backend test`
   - Run complete frontend check: `pnpm --filter frontend build && pnpm --filter frontend test --no-watch`
4. **TDD Cycles**: Before implementing a feature or fixing a bug, write unit/integration tests that describe the expected behavior, execute the test to watch it fail (if applicable), write the code, and confirm the entire test suite stays green.
